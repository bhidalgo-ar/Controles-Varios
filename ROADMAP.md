...
| F4 | `fileTypes.js` con un mapa único, config declarada en el registry, matar el `Promise.all` posicional | en curso — Paso 0 (`Promise.all` por clave) hecho. El resto se replantea como **contrato de export**: plan por pasos en `specs/contrato-export.md`. Pasos 0-6 hechos (2026-08-12/13) — `js/exports/contractSheet.js` (`writeContractSheet` + `writeGroupedContractSheet`), `unitsEvaluated` en Brutos/GS Pers (D-043), y los 5 contratos que faltaban + el fix de "un contrato es piso, nunca techo" (D-045). El asiento de FINADIET entró después con sus dos contratos ya escritos por `writeContractSheet` (D-046). Los writers del Paso 6 migraron el 2026-08-13 — 4 de 5, `writeContractSheet`/`writeGroupedContractSheet` ganaron fila de TOTAL y filas atenuadas; Acreditaciones se queda con su .xlsx a mano (D-047). Queda: el `fileTypes.js` original |
...
| 3.9 | Asiento de Remuneraciones (FINADIET) | 2 | M | hecho ✅ (2026-08-12) — control `finadiet_asiento`, ver `specs/finadiet-asiento-remuneraciones.md` y D-045. Pendiente: correrlo contra el archivo real del cliente y confirmar con Gaby que da lo mismo que el armado a mano de un mes ya cerrado |
