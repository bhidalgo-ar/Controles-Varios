...
| **Merz** | Axton | El piloto del adaptador Axton | **Desbloqueado (2026-08-20):** novedades y Tabulado de 07/2026 relevados (D-070); N0a, N0b, N1 y N2 ya están construidos. Merz queda como piloto de la familia de Novedades junto con SIASA — cliente chico (43 legajos) que guarda ORIGINAL/MODIFICADO de sus planillas | Bajo: el trabajo ahora es conseguir un importador + Tabulado + Totales de Concepto reales de un mismo mes para verificar N1 y N2, no construir nada |
| **SIASA** | Axton | Verificar N1 y N2 punta a punta contra 07/2026 (guarda las 3 capas: recibida / modificada / importador, por 4 UOs). N2 todavía no vio ningún archivo real | Preguntar a la analista por el legajo que está en la planilla recibida de Aguas y Gaseosas 07/2026 y no llegó al importador — es el primer caso de verificación de N2 (D-064); y conseguir un importador + Tabulado + Totales de Concepto reales de una UO del mismo mes | Medio: sin esa respuesta y esos archivos, el primer caso del cruce no se puede cerrar |
...
|---|---|---|---|---|
| 3.12 | ~~**Familia de Novedades (Axton) — N0a: lector ExpNov**~~ **hecho (2026-08-20)** | **1** | M | Cimiento: parser de la familia de archivos de novedades/importador (`d  axFiles ...ExpNov...`), reconoce por firma y nunca por posición, separa `cantidad$importe`, lista aparte las columnas sin código. Formato relevado en los 7 clientes Axton el 2026-08-20 (D-070, `specs/familia-novedades-axton.md`). Prompt de arranque en `docs/prompts-familia-novedades.md` |
| 3.13 | ~~**Familia de Novedades — N0b: parser Axton del Tabulado + Totales de Concepto**~~ **hecho (2026-08-20)** | — | — | `js/parsers/tabAxtonReader.js` + `readTotalesConcepto`: preámbulo 0/1/2 filas, Cant/Imp vs sólo-Imp, `TOTAL GENERAL` simple o duplicado (corta en el ÚLTIMO), fila por liquidación sin consolidar (eso es del control), filas manuales post-total avisadas, U+00A0, validación de sumas contra el archivo. El campo `Reporte:` entró al detector con el cuarto formato `axton_tot`. Ver **D-072**. **Pendiente:** correrlo contra un Tabulado real; y que `tabAxtonParser.js` (el estricto, que usa Variaciones de POP) pase a delegar en el lector |
| 3.14 | ~~**Familia de Novedades — N1: generador de importador**~~ **hecho (2026-08-20)** | **1** | L | La app arma el `F2_Consolidada` por UO desde la planilla del cliente; el analista valida en pantalla y descarga. Disuelve B2a por diseño (D-070). Control `novedades_importador`, con casillero opcional para controlar lo generado contra el importador ya armado. **Falta la verificación contra archivos reales** (el layout del F2 y un caso completo de SIASA 07/2026, D-064) — detalle en `specs/familia-novedades-axton.md` § "Lo que N1 espera de un archivo real" |
| 3.15 | ~~**Familia de Novedades — N2: novedades vs liquidación (B2b)**~~ **hecho (2026-08-20)** | **1** | L | Importador vs Tabulado + Totales de Concepto, por legajo+código, consolidando los **tres** lados (`expNovParser.js`, `tabAxtonReader.js` + `readTotalesConcepto`). Cantidad e importe cuando ambos existen; cuatro bandas —coincide / difiere / no comparable / sin contraparte— y lo no comparable informa, no bloquea (D-070). Control `novedades_liquidacion`. Tres decisiones de criterio abiertas a confirmación en **D-073**. **Falta la verificación contra archivos reales** — no vio ninguno todavía; primer caso: el legajo que en SIASA Aguas y Gaseosas 07/2026 está en la planilla del cliente y no llegó al importador — detalle en `specs/familia-novedades-axton.md` § "Lo que N2 espera de un archivo real" |
