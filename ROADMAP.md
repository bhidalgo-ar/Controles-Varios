...
|---|---|---|---|---|
| 3.12 | **Familia de Novedades (Axton) — N0a: lector ExpNov** | **1** | M | Cimiento: parser de la familia de archivos de novedades/importador (`d  axFiles ...ExpNov...`), reconoce por firma y nunca por posición, separa `cantidad$importe`, lista aparte las columnas sin código. Formato relevado en los 7 clientes Axton el 2026-08-20 (D-069, `specs/familia-novedades-axton.md`). Prompt de arranque en `docs/prompts-familia-novedades.md` |
...
| 3.14 | **Familia de Novedades — N1: generador de importador** | **1** | L | La app arma el `F2_Consolidada` por UO desde la planilla del cliente; el analista valida en pantalla y descarga. Disuelve B2a por diseño (D-069). Piloto: SIASA (guarda las 3 capas del circuito) y Merz. Depende de N0a |
| 3.15 | **Familia de Novedades — N2: novedades vs liquidación (B2b)** | **1** | L | Importador validado vs Tabulado + Totales de Concepto, por legajo+código, consolidando los dos lados. Cantidad e importe cuando ambos existen; lo no comparable informa, no bloquea (D-069). Piloto: SIASA y Merz; volumen: POP. Depende de N0a+N0b+N1. Primer caso de verificación: el legajo 12→11 de SIASA Aguas y Gaseosas 07/2026 |
...
| 3.10 | **Umbrales de verdad globales y editables** | 3 | M | Hoy el bloque "Umbrales" del panel lateral (`thresholdsSectionHtml` en `js/ui/controlsWizard.js`) muestra "$ 1,00 / 0,1 %" **escritos a mano, que ningún control lee**: cada control trae su tolerancia cableada (~47 `0,01` repartidos por `js/controls/`). Lo levantó Willy el 2026-08-19 al ver dos cifras distintas en la misma pantalla. Arreglo mínimo ya hecho: un control con tolerancia propia declara `ownThresholdNote` en el registry y el bloque se apaga explicando de dónde sale el número (hoy sólo el Control de Netos). Lo que falta: que el umbral se edite ahí, se guarde por cliente y lo lean los 18 controles — y ahí se borran `ownThresholdNote` y sus notas. **No confundir con el umbral del semáforo** (`semaforoThresholdPct`, default 2 %), que sí funciona y mide otra cosa: qué porcentaje de legajos con diferencia pinta la tarjeta de rojo |
