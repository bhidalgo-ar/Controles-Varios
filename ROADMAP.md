...
| F4 | `fileTypes.js` con un mapa único, config declarada en el registry, matar el `Promise.all` posicional | en curso — Paso 0 (`Promise.all` por clave) hecho. El resto se replantea como **contrato de export**: plan por pasos en `specs/contrato-export.md`. Pasos 0-3 (2026-08-12), 4a/5 (2026-08-13) y 4b (2026-08-12) hechos — `js/exports/contractSheet.js` (`writeContractSheet` + `writeGroupedContractSheet`), `unitsEvaluated` en Brutos/GS Pers (D-043). Del Paso 6 ya entraron los dos contratos del asiento de FINADIET (`finadiet_asiento_cc`/`finadiet_asiento_gral`, D-045); faltan los de los controles restantes |
