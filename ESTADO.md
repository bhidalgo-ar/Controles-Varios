...
- Próximo paso: queda **N2** (depende de N0a + N0b + N1, los tres hechos), con el prompt, el modelo y el esfuerzo en `docs/prompts-familia-novedades.md`. Antes: correr los dos lectores contra archivos reales —una planilla de novedades de SIASA o de Merz, y un Tabulado de cada cliente— porque los del relevamiento no entraron al repo y ahí puede aparecer una firma no vista. Queda como deuda que `tabAxtonParser.js` —el lector estricto de Tabulado que hoy usa Variaciones de POP— pase a delegar en `tabAxtonReader.js`, en un PR aparte. De Willy: una planilla y su F2 real de una UO de SIASA 07/2026 para cerrar la verificación de N1; abrir en el navegador las planillas bloqueadas por etiqueta de confidencialidad (Epiroc, Red Bull, POP templates); y preguntar a la analista de SIASA por el empleado que quedó fuera del importador en Aguas y Gaseosas.
- Detalle: `specs/familia-novedades-axton.md`, D-070, D-071 (N1), **D-072** (N0b).
