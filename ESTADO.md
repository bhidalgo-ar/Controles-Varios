...
## Familia de Novedades (Axton) — los dos cimientos construidos, faltan N1 y N2
...
- **N0a hecho** (mismo día): `js/parsers/expNovParser.js` lee la familia ExpNov por firma —la fila con "Legajo" y "Apellido y Nombres", los códigos pegados a ella— y devuelve una novedad por celda cargada (legajo crudo, código, cantidad, importe, unidad declarada), la lista de columnas sin código con su rótulo y cuántas celdas tienen, la metadata de UO/empresa y los avisos (código duplicado, código no numérico, fila sin legajo, valor ilegible, hojas no leídas). El período no sale del archivo. 68 asserts en `tests/expNovParser.test.js`, en la cadena de `package.json`. **Ningún control existente cambia y todavía no hay pantalla**: es la pieza que van a consumir N1 y N2. Falta probarlo contra un archivo real —los del relevamiento no entraron al repo—, y ahí puede aparecer alguna variante de firma no vista.
...
- Próximo paso: los 2 chats que faltan — N1 y N2 — con los prompts, modelo y esfuerzo ya escritos en `docs/prompts-familia-novedades.md`. Antes de N1, correr el lector de novedades contra una planilla real de SIASA o de Merz y confirmar que la reconoce; ídem el lector de Tabulado contra un Tabulado real de cada cliente (los del relevamiento no entraron al repo, así que puede aparecer una firma no vista). Queda como deuda que `tabAxtonParser.js` —el lector estricto que hoy usa Variaciones de POP— pase a delegar en el nuevo, en un PR aparte. De Willy: abrir en el navegador las planillas bloqueadas por etiqueta de confidencialidad (Epiroc, Red Bull, POP templates) y preguntar a la analista de SIASA por el empleado que quedó fuera del importador en Aguas y Gaseosas.
- Detalle: `specs/familia-novedades-axton.md`, D-070, **D-071**.
...
- Detalle: `specs/lector-tabulado-formatos.md`, D-065, **D-071**.
