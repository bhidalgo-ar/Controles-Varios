...
- **Reportado y no arreglado, para decidir:** `tabIdCentroTrabColumn` y `tabIdCategoriaColumn` las consume el contrato de `nr_reporte` y las completa sola la auto-detección, pero no están en el panel "Columnas del Tabulado" — si se equivoca, el analista no tiene dónde corregirlas. Y la zona de drop de Acumuladores dice "Acumuladores (Axton)" mientras la etiqueta del tipo dice "Acumuladores (export de Axton)": la divergencia es anterior a la fase y quedó declarada y fijada por un assert en vez de unificada de prestado.
