...
- **`audience: 'finanzas'` y D-020:** el archivo lo recibe Contaduría del cliente, así que no lleva legajo, nombre ni atributos del empleado. `tests/exportContracts.test.js` pasa de asertar "todavía no hay ningún contrato finanzas" a hacer cumplir la regla real (ningún contrato `finanzas` con puesto, categoría, centro de trabajo, altas/bajas o dotación).
