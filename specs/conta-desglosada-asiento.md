...
`tests/contaDesglosadaControl.test.js` (114 asserts). Verificado el 2026-08-19 contra los dos
...
quedan como están. Quedó afuera, a la espera del porqué de Contaduría, la línea que vino sin importe
en el origen: sus dos lados se dejaron vacíos.
**El 2026-09-16 cae esa excepción** (D-096 revisada): llegó el porqué —el archivo no se importa a
ningún sistema, lo trabaja Contaduría a mano con fórmulas, y una celda vacía en un rango es lo
incómodo— y con eso la línea sin importe también sale con sus tres importes (Importe, DEBE y HABER)
en `0,00`. En el `.xlsx` ya no queda ninguna celda de importe vacía; la señal de que a una línea le
falta el dato queda sólo en pantalla (`filasSinImporte`) y en las líneas del control (`null`).
...
**En el archivo de la desglosada, Importe, DEBE y HABER salen en `0,00` y no vacíos, sin excepción**
(D-096, pedido de Contaduría del cliente vía Mica). Para casi todas las líneas el `0,00` no reemplaza
a un dato que falta: cada línea de la desglosada lleva importe de un solo lado por construcción
(regla 1, §2), así que el lado sin importe siempre significa "de este lado no va nada". La línea que
sí pierde algo es la que vino sin importe en el origen: sus tres importes nacen en `null` porque falta
el dato, y desde el 2026-09-16 salen en `0,00` igual que una línea que no mueve plata. Se decidió a
sabiendas, con el porqué de Contaduría: el archivo no se importa a ningún sistema, lo trabaja
Contaduría a mano con fórmulas, y ahí una celda vacía en el medio de un rango es lo incómodo —no es
un problema de que rompa una importación. Lo que queda como señal de que a una línea le falta el dato
es la pantalla de resultados (`filasSinImporte`, que el control sigue contando y avisando) y las
líneas del control, que siguen en `null`; en el `.xlsx` ya no se distingue "no vino el dato" de "vale
cero". Se escribe en el borde del export —`lineasDelArchivoDesglosada(results)`—, así que ni el
cálculo, ni la pantalla del control, ni el Asiento Contable lo ven: el Asiento sigue con la celda
vacía en el lado sin importe.
