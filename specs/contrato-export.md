...
> (D-049): no migra al writer, pero su contrato dejó de ser una declaración sin quien la haga cumplir.
...
1. **Estructural: no se puede escribir un export a mano.** `writeContractSheet(wb, contract, rows,
   ...
   **Corrección de alcance (D-049):** "nadie más hace `addRow`" no se cumple universalmente y no era
...
queda afuera. Ojo: el relevamiento de D-049 corrigió esa lectura — ni multi-hoja ni las fórmulas entre
...
`headerRows`) en `js/exports/contracts.js` y entraron a `CON_WRITER` (que desde D-049 vive ahí mismo, no en el test).

**`acreditaciones_reporte` se queda sin writer, a propósito** — cerrado el 2026-08-13 como **excepción
permanente declarada**, no como deuda (D-049). Al ir a cerrarlo se corrigieron las dos mitades del
planteo de D-047:
...
vacuidad cambiando el alto del encabezado y viéndola fallar. Detalle completo en D-049.
