...
| **Evals** | Método: prueba automática (`tests/dbMigrationV4.test.js`, corre en CI) que arma una base v3 con varios clientes de prueba (incluyendo 2-3 con nombres que colisionan en el slug, para probar el sufijo, y clientes con groupers/sesiones/corridas/catálogo colgando) y corre el `upgrade()` real de Dexie. Criterio: todos quedan con `code` único y `sourceSystem: 'meta4'`; ningún grouper, sesión, corrida o catálogo cambia de dueño o desaparece. Revisa: el agente reporta en español llano; el usuario confirma mirando el CI en verde. Riesgo residual conocido (ver acuerdo transversal §0): un dato real con una forma que el agente no anticipó puede no estar cubierto — se trata como bug si aparece, no bloquea el merge. |
...
| **Evals** | Método: prueba automática (corre en CI) que simula dos navegadores distintos importando el mismo archivo de seed — uno de ellos con corridas locales previas. Más una prueba de extremo a extremo con navegador real (Playwright). Criterio: ambos quedan con los mismos 22 clientes y los mismos `code`; el que tenía corridas previas las conserva después de importar. Revisa: el agente reporta en español llano; el usuario confirma con el CI en verde. |
...
| **Condición de salida** | Para cuando las pruebas automáticas y la de extremo a extremo pasan en CI. No implementa todavía edición desde la app de lo importado (eso es T6) ni toca `controlConfigs`. |
...
| **Scope** | Entra: campos `scope`/`scopeMeta`/`appliesWhen` en las 10 entradas del registry con default no-op (`scope:'general'`, `appliesWhen: () => true`); sección "Aplican a este cliente" / "Otros controles" (colapsada, no oculta, solo se renderiza si hay algo que mostrar ahí) en el wizard. **Ajuste del 2026-07-30:** los "3 predicados reales" que este renglón preveía originalmente (`f1359`→Marval, `pluriempleo`→Sportline/Lowsedo, `paymentUsd`→Geopagos/Piano) no se implementaron — ninguno de los 10 controles reales del registry corresponde a esos atributos (F.1359 es un control hipotético usado como ejemplo en `ARCHITECTURE.md` §4, no uno de los 10 existentes). Atarle esos atributos a un control real existente sin saber si corresponde de verdad hubiera sido inventar una regla de negocio — se le preguntó al usuario cuál de los 10 controles debía restringirse y no hubo respuesta, así que se avanzó con el mecanismo solo (opción por default: no romper nada hoy). Afuera: no se agregan predicados especulativos; no se toca `controlConfigs` (T5). |
| **Evals** | Método: `tests/controlsRegistryScope.test.js` (CI): confirma que los 10 controles tienen `scope`/`scopeMeta`/`appliesWhen`, y que `appliesWhen` da `true` para cualquier cliente (con o sin atributos activados) — o sea, cero cambio observable hoy. Más `tests/e2e/controlsWizardScope.spec.js` (CI, Playwright): abre el wizard de un cliente cualquiera y confirma que aparecen los mismos 10 controles de siempre y que no se renderiza la sección "Otros controles" (porque nada la necesita todavía). Revisa: el agente reporta en español llano; el usuario confirma con el CI en verde. |
| **Autonomía** | Decide solo: texto de los encabezados de sección, estructura interna del filtro. Consulta antes de: atar cualquier predicado a un control real existente — eso ya se preguntó una vez (ver Scope) y quedó pendiente de que el usuario indique cuál, si alguno, corresponde. |
| **Condición de salida** | Para cuando los dos tests de arriba pasan en CI. No se inventan predicados para los 10 controles existentes; el ejemplo de predicados (f1359/pluriempleo/paymentUsd) queda documentado en el comentario de `registry.js` para cuando exista un control real que los necesite. |
...
| **Comportamientos a preservar** | Un analista con `brutos_tab_config`, `rendvstabu_concept_grouping` o `rva_config` ya guardados en `fileProfiles` (v4) tiene que ver el control arrancar con la misma configuración después de migrar a v5 — verificado con una prueba automática (ver Evals). |
...
| **Evals** | Método: prueba automática (`tests/dbMigrationV5.test.js`, corre en CI) que arma una base v4 con las 3 claves viejas configuradas en `fileProfiles` para clientes de prueba y corre el `upgrade()` real. Criterio: el control correspondiente (Brutos, Rend vs Tabu, o Rend vs Asiento) queda con la misma configuración en `controlConfigs` que tenía antes de migrar. Revisa: el agente reporta en español llano; el usuario confirma mirando el CI en verde. Mismo riesgo residual conocido que T2 (acuerdo transversal §0). |
...
| **Condición de salida** | Para cuando `tests/dbMigrationV5.test.js` pasa en CI para al menos una de las 3 claves migradas. No limpia `fileProfiles` de las claves viejas, no construye la UI de edición de `controlConfigs` desde admin (eso es T6). |
...
| **Evals** | Método: prueba de extremo a extremo (Playwright, corre en CI) que simula dos navegadores: uno edita algo en admin y exporta, el otro importa ese archivo (reusando el flujo de T3). Criterio: el cambio llega al segundo sin perder sus corridas locales. Revisa: el agente reporta en español llano; el usuario confirma con el CI en verde. |
...
| **Condición de salida** | Para cuando la prueba de export→import entre dos navegadores pasa en CI. No se agrega nada de T7/T8/T9/T10 aunque el modo admin sea un lugar tentador para "dejar todo preparado". |

---

**Fecha de creación:** 2026-07-30 · **Actualizada:** 2026-07-30 (evals: pruebas automáticas + CI reemplazan la verificación manual en navegador real como criterio de cierre, ver acuerdo transversal §0)
**Confirmada por el usuario:** sí — T0 y T1 ya ejecutadas y mergeadas (#54, #55).
