...
test.describe('Cruce por Agrupadores — ficha por legajo', () => {
  ...
  test('"Marcas ▾" trae los agrupadores y filtra la lista', async ({ page }) => {
    // El desplegable `Marcas ▾` compartido (`createMarcasFilter` de tableTools):
    // la solapa Fichas tenía una copia local con `data-ficha-marca` y pasó a usar
    // la pieza de la barra, que es la misma que monta la Planilla.
    const marcas = page.locator('[data-marca-filter]');
