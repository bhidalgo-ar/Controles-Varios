...
## D-096 — En el archivo de la desglosada, DEBE, HABER e Importe salen en 0,00 y no vacíos: excepción acotada a "`null` no es `0`"

**Fecha:** 2026-09-15. **Revisada:** 2026-09-16 (cae la excepción que dejaba una línea con los dos lados
vacíos — ver más abajo).

**Contexto (2026-09-15):** Contaduría del cliente (COTY) pidió, vía Mica, que en la Contabilidad
Desglosada el lado de DEBE o HABER que no lleva importe salga con `0,00` en vez de la celda vacía.
Quedó `PENDIENTE: falta el porqué` de Contaduría — no se había registrado si era para que cerrara una
fórmula del lado de ellos, un requisito de su sistema contable, u otra cosa.

**Decisión original (2026-09-15):** se agregó `lineasDelArchivoDesglosada(results)` en
`js/controls/contaDesglosada.js`, que mapea las líneas del cálculo (`lineasDeLaDesglosada`) **en el
borde del export**: donde antes un lado sin importe quedaba `null`, pasó a salir `0`. La usan los tres
formatos de la desglosada — `.xlsx`, CSV y "Copiar" — y **sólo** ellos: el Asiento Contable sigue con
la celda vacía, y la pantalla del control tampoco cambia, porque ninguna de las dos lee
`lineasDelArchivoDesglosada`.

Se planteó como excepción **acotada** al gotcha "`null` no es `0`" del `CLAUDE.md`, no como derogación:
cada línea de la desglosada lleva el importe de un solo lado **por construcción** (regla 1,
desdoblamiento, §2 de la spec), así que el lado que no lo lleva no es un dato que falte — es la mitad
de la línea que no correspondía escribir. `0,00` ahí no tapa nada, describe correctamente "de este
lado no va nada".

Como en ese momento no estaba el porqué de Contaduría, se dejó afuera la única línea donde el vacío
**sí** es falta de dato: la que vino sin importe en el origen, con los dos lados —DEBE y HABER— en
`null`. Sin saber para qué necesitaban el cero del otro lado, no había forma de pesar ese pedido
contra el costo de D-036 (una línea sin dato completada con cero no la detecta nadie), así que se
optó por conservarla vacía.

**Revisión (2026-09-16): cae la excepción.** Willy trajo el porqué que faltaba: el archivo de la
desglosada **no se importa a ningún sistema** — es sólo para control y quizás reportería. No es que
una celda vacía rompa una importación; es que Contaduría del cliente lo trabaja a mano, con fórmulas,
y ahí una celda vacía en el medio de un rango es justamente lo incómodo. Con ese motivo, el costo de
D-036 deja de pesar más que el pedido — nadie va a leer un `0,00` puesto ahí como "el dato existe y
vale cero", porque no hay ningún sistema aguas abajo que lo interprete: lo lee una persona, a mano.

Se completan entonces los dos lados de la línea sin importe, y de paso se suma una tercera columna que
antes no se tocaba: **Importe**. Las tres columnas de plata van **juntas** a partir de acá — una fila
con DEBE y HABER en cero pero Importe vacío se contradice sola. `lineasDelArchivoDesglosada` pasa a
completar `importe`, `debe` y `haber` **sin excepción**.

El precio, asumido a sabiendas: en el `.xlsx` ya no se distingue "no vino el dato" de "vale cero". La
señal que antes daba la celda vacía queda sólo en la pantalla de resultados — el control sigue
contando esas filas y avisándolas (`filasSinImporte`) — y en las líneas del control (`r.lineas`), que
...
- **Escribir el cero en el cálculo** (`armarDesglosada`, donde nace cada línea) en vez de en el export:
  se descartó porque ahí adentro `debe`/`haber`/`importe` siguen siendo la fuente para la pantalla del
  control y para el Asiento (`lineasDeLaDesglosada` es la base de los dos), y los dos tienen que seguir
...
**Motivo:** pedido explícito de Contaduría del cliente, vía Mica, con el porqué que trajo Willy el
2026-09-16: el archivo no se importa a ningún sistema, se trabaja a mano con fórmulas, y una celda
vacía en un rango es lo que complica esa forma de trabajo. Ya no queda pendiente.
